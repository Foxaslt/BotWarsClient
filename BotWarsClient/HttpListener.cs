...
namespace BotWarsClient
{
    public class HttpListenerClass
    {
                ...
                case "move":
                    {
                        if (context.Request.HttpMethod.ToLower() == "get")
                        {
                            ...
                            _bot.MyMoveHistory.Add(move);
                            string responsestr = Enum.GetName(typeof(Move), move);
                            Console.WriteLine("My move {0}", responsestr);
                        ...
                        else
                        {
                            ...
                            if (!Enum.TryParse(body.Split('=').Last(), out lastMoveVal))
                            {
                                lastMoveVal = Move.Invalid;
                            }
                            _bot.CaptureOpponentsLastMove(lastMoveVal);
                            Console.WriteLine("Their move {0}", body);
                            context.Response.StatusCode = (int)HttpStatusCode.OK;
                            context.Response.ContentType = "text/plain";
                            StreamWriter sw = new StreamWriter(context.Response.OutputStream);
                            using (sw)
        ...
        private void ProcessStartResponse(string responseBody)
        {
            ...
            Console.WriteLine("START Opponentname={0} Health={1} ArenaSize= {2} Flips={3} FlipOdds={4} Direction={5} Fuel={6} StartPosition={7}", opponentName, health, arenaSize, flips, flipOdds, direction, fuel, startPosition);
        ...
        private void Initialize(int port)
        {
            ...
            _serverThread.Start();
            Console.WriteLine("Bot initialised on port {0}.", port);
        }
